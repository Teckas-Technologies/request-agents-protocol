    ...
    return (
        <div className="header fixed z-50 w-full bg-[#111521] h-[6.5rem] flex items-center justify-between 2xl:px-[6rem] xl:px-[5rem] md:px-[4.2rem] px-4">
            ...
            <div className="header-center">
                <div className="navs flex items-center h-full gap-[2rem] md:flex hidden">
                    ...
                    <div className="relative create-nav w-auto h-[5rem] flex items-center justify-center cursor-pointer" onClick={() => router.push("/")}>
                        <h2 className={`${pathName === "/" ? "text-[#ABBDFE]" : "text-[#D7D7D7] dark:text-[#D7D7D7]"}`}>Create an Agent</h2>
                    </div>
            ...
            <div className="header-right flex items-center gap-2">
                ...
                {isConnected && address && <div className="explorer-btn md:flex hidden gap-2 items-center px-6 py-2.5 rounded-lg cursor-pointer" onClick={() => window.open(`https://scan.request.network/address/${address}`, '_blank')}>
                    <h2 className="font-medium text-[#ABBDFE]">Explorer</h2>
                    <InlineSVG
                        src="/icons/goto.svg"
                        className="fill-current bg-transparent text-[#ABBDFE] w-3 h-3"
                    />
                </div>}
            ...
            {sideMenu && <div className={`absolute h-[100vh] w-full top-[6.5rem] left-0 bg-[#111521] md:hidden`}>
                <div className="side-menus flex flex-col gap-5 mt-3 py-[1rem] px-[2rem]">
                    ...
                    <div className="menu" onClick={() => { setSideMenu(false); router.push("/"); }}>
                        <h2 className="text-white text-lg font-medium pb-1">CREATE AN AGENT</h2>
                    </div>
                    ...
                    {isConnected && address && <div className="explorer-btn flex gap-2 items-center justify-center px-6 py-2.5 rounded-lg cursor-pointer" onClick={() => window.open(`https://scan.request.network/address/${address}`, '_blank')}>
                        <h2 className="font-medium text-[#ABBDFE]">Explorer</h2>
                        <InlineSVG
                            src="/icons/goto.svg"
                            className="fill-current bg-transparent text-[#ABBDFE] w-3.5 h-3.5"
                        />
                    </div>}
                </div>
            </div>}
